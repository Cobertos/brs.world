...
export class BRSMesh extends THREE.Mesh {
  ...
  constructor (brsjsData) {
    const [geo, mat] = BRSMesh._buildResources(brsjsData);
    super(geo, mat);
  ...
  static fromBuffer (arrayBuffer) {
    ...
    const brsjsData = BRS.read(arrayBuffer);
  ...
  static _buildResources (brs) {
    ...
    brs.bricks.forEach((br, idx) => {
      ...
      const pos = new THREE.Vector3(br.position[0], br.position[2], br.position[1]);
      const axes = [
      ...
      const quat = new THREE.Quaternion().setFromAxisAngle(axes[br.direction], rots[br.rotation]);
      ...
      const scale = new THREE.Vector3(br.size[0] * 2, br.size[2] * 2, br.size[1] * 2);
      const color = Array.isArray(br.color) ? new THREE.Color(br.color[0] / 255, br.color[1] / 255, br.color[2] / 255)
        : brs.colors[br.color];
    ...
    const instancedMaterial = new InstancedStandardMaterial({
